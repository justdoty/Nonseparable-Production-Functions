...
expb <- function(idvar, X, U, MH, par1, parL, WT){
	if (!missing(X)){
		b1 <- -mean(apply(U, 2, function(u) mean(X[,1]<=cbind(1, X[,-1], u)%*%par1)/mean((X[,1]-cbind(1, X[,-1], u)%*%par1)*(X[,1]<=cbind(1, X[,-1], u)%*%par1))))
		bL <- mean(apply(U, 2, function(u) mean(X[,1]>cbind(1, X[,-1], u)%*%parL)/mean((X[,1]-cbind(1, X[,-1], u)%*%parL)*(X[,1]>cbind(1, X[,-1], u)%*%parL))))
	} else {
		data <- lagdata(idvar, U)
		names(data) <- c("idvar", paste("Ucon", 1:ncol(U), sep=""), paste("Ulag", 1:ncol(U), sep="")) 
	    Xpar1 <- apply(as.matrix(data[, grepl("Ulag", names(data))]), 2, function(u) cbind(1,u)%*%par1)
		XparL <- apply(as.matrix(data[, grepl("Ulag", names(data))]), 2, function(u) cbind(1,u)%*%parL)
