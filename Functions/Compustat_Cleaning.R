...
wind <- 0.01
compstat <- read.csv('/Users/justindoty/Documents/Research/Dissertation/Nonlinear_Production_Function_QR/Data/compustat.csv', header=TRUE) %>% rename(id=gvkey, year=fyear, employ=emp, rd=xrd, adv=xad) %>% 
	...
	select(id, year, sale, employ, ppegt, ppent, cogs, xsga, capx, naics, oibdp, dp, dpact, rd) %>% group_by(id) %>% mutate(age=year-first(year)+1) %>% ungroup() %>%
	...
	select(id, year, sale, employ, ppegt, ppent, cogs, xsga, capx, oibdp, dp, dpact, age, yprice, lprice, mprice, iprice, naics, naics3, naics2, rd) %>% filter(!is.na(dpact), !is.na(dp)) %>%
	...
	select(id, year, sale, employ, ppegt, ppent, cogs, xsga, capx, oibdp, age, yprice, lprice, mprice, iprice, kprice, naics, rd) %>%
	#Put variable into comparabe units (in thousands)
	mutate(sale=sale*1e3, ppegt=ppegt*1e3, ppent=ppent*1e3, capx=capx*1e3, employ=employ, lexp=lprice, oibdp=oibdp*1e3, rd=rd*1e3) %>%
	...
	mutate(Y=sale/yprice, K1=ppegt/kprice, K2=ppent/kprice, L=employ, I=capx/iprice, lexp=employ*lprice, oibdp=oibdp, RD=ifelse(is.na(rd), 0, rd)) %>% filter(cogs>0, xsga>0) %>%	
	...
	group_by(id) %>% filter(Y>0, !is.na(Y), K1>0, !is.na(K1), K2>0, !is.na(K2), L>0, !is.na(L), M>0, !is.na(M), I>0, !is.na(I), VA>0, !is.na(VA), RD>=0) %>% 
	filter(year>=1997) %>%
	select(id, year, Y, VA, K1, K2, L, M, I, age, naics2, RD)
...
USest <- compstat %>% transmute(id=id, year=year, lny=log(Y), lnva=log(VA), lnk1=log(K1), lnk2=log(K2), lnl=log(L), lnm=log(M), lni=log(I), age=age, rd=RD, naics2=naics2)
####################################################################################################
