...
RX <- function(K, omega){
	Rdat <- cbind(K, omega)
	RX <- cbind(1, tensor(M=c(3,3), vars=Rdat)[,-1])
	return(RX)
}
#Derivative of Investment Rule
RXD <- function(K, omega, par, pos, sdpos){
	M <- c(3,3)
	Rdat <- cbind(K, omega)
	dten <- dtensor(M=M, vars=Rdat, pos=pos, sdpos=sdpos)
	RXD <- dten$prod%*%par[dten$dpos,]
	return(RXD)
}
#Productivity Process for t>1 
WX <- function(omega, R, Rind){
	WXNR <- cbind(omega, omega^2, omega^3)
	WXR <- tensor(M=c(3,3), vars=cbind(omega, R))[,-1]
	WX <- cbind(1, c(!Rind)*WXNR, c(Rind)*WXR)
	return(WX)
}
WXRD <- function(omega, R, Rind, par, pos, sdpos){
	parNR <- par[2:4,]
	parR <- par[c(1, 5:nrow(par)),]
	WXNRD <- cbind(1, 2*omega[!Rind], 3*(omega[!Rind])^2)%*%parNR
	dten <- dtensor(M=c(3,3), vars=cbind(omega[Rind], R[Rind]), pos=pos, sdpos=sdpos)
	WXRD <- dten$prod%*%parR[dten$dpos,]
	return(list(d1=WXNRD, d2=WXRD))
}
