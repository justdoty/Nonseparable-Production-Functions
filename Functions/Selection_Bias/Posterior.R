...
densq <- function(idvar, YX, vectau, pred, parb1, parbL){
	...
	# Density for Productivity
	denswt <- function(idvar, YX, g1, gL, vectau, pred, parb1, parbL){
	densq <- suppressWarnings(ifELSE((pred[,1]<YX)&(pred[,2]>=YX), log(vectau[2]-vectau[1])-log(pred[,2]-pred[,1]), 0))
	for (q in 2:(length(vectau)-1)){ 
		densq <- suppressWarnings(ifELSE((pred[,q]<YX)&(pred[,q+1]>=YX), log(vectau[q+1]-vectau[q])-log(pred[,q+1]-pred[,q]), densq))
	}
	densq <- suppressWarnings(ifELSE(YX<=(pred[,1]+g1), log(vectau[1])+log(parb1)+parb1*(YX-pred[,1]-g1), densq))
	densq <- suppressWarnings(ifELSE(YX>(pred[,length(vectau)]+gL), log(1-vectau[length(vectau)])+log(parbL)-parbL*(YX-pred[,length(vectau)]-gL), densq))
	densq <- data.table(id=idvar, densq=densq)[,sum(densq),keyby=id]$V1
	return(densq)
	}
...
###############################################################################################################
posterior <- function(ydata, inpdata, wtdata, w1data, vectau, par, method){
	...
	#Probability of Staying in Operation
	Phat <- predict(par$omegabar, type="response", data=wtdata)
	densWT <- denswt(idvar=wtdata$idvar, YX=wtdata$Wcon, g1=predict(par$g1, wtdata), gL=predict(par$gL, wtdata), vectau=vectau, pred=(WX(omega=wtdata$Wlag)%*%par$resWTinit), parb1=(par$wtbinit)$b1, parbL=(par$wtbinit)$bL)
	Phatstay <- 1-Phat
	densP <- data.table(id=wtdata$idvar, Phat=Phatstay)[,sum(Phat), keyby=id]$V1
	#Final posterior
	finaldens <- densY+densL+densM+densI+densWT+densW1+densP
}
######################################################################################################
######################################################################################################
######################################################################################################
