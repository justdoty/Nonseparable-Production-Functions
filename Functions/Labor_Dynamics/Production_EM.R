...
require(data.table)
require(truncnorm)
source('NLPFQR/DATA/Labor/Tensors.R')
source('NLPFQR/DATA/Labor/Posterior.R')
source('NLPFQR/DATA/Labor/Mstep.R')
source('NLPFQR/DATA/Labor/Auxfuns.R')
source('NLPFQR/DATA/Labor/omega.R')
Production_EM <- function(ntau, idvar, timevar, Y, A, K, L, M, I, RD, method="trans", maxiter, draws){
	...
	ldata <- data.frame(idvar=idvar[idcon], timevar=timevar[idcon], Lcon=L[idcon], Kcon=K[idcon], Llag=L[idlag], Wcon=W[idcon])
	...
	resLinit <- rq(Lcon~LX(K=Kcon, L=Llag, omega=Wcon)-1, tau=vectau, data=ldata)
	...
	for (iter in 1:maxiter){
		...
		ydata$WY <- omegadat
		inpdata$WX <- omegadat
		ldata$Wcon <- omegadat[idcon]
		wtdata$Wcon <- omegadat[idcon]
		wtdata$Wlag <- omegadat[idlag]
		w1data$W1 <- omegadat[!idcon]
		#Posterior density
		oldpost <- posterior(ydata=ydata, inpdata=inpdata, ldata=ldata, wtdata=wtdata, w1data=w1data, vectau=vectau, par=resinit, method=method)
		#Time the E-step
		etime <- proc.time()
		for (j in 1:draws){
			...
			newpost <- posterior(ydata=ydata, inpdata=inpdata, ldata=ldata, wtdata=wtdata, w1data=w1data, vectau=vectau, par=resinit, method=method)
		...
		#Labor
		lbinit <- expb(YX=ldata$Lcon, pred=predict(resLinit, ldata))
		...
		#Labor
		lw <- colMeans(LXD(K=ldata$Kcon, L=ldata$Llag, omega=ldata$Wcon, par=matrix(as.numeric(resLinit$coef), ncol=ntau), pos=3, sdpos=1))
		#Materials
