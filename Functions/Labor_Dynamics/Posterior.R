...
###############################################################################################################
posterior <- function(ydata, inpdata, ldata, wtdata, w1data, vectau, par, method){
	...
	densL <- densq(idvar=ldata$idvar, YX=ldata$Lcon, vectau=vectau, pred=predict(par$resLinit, ldata), parb1=(par$lbinit)$b1, parbL=(par$lbinit)$bL)
	...
	#Final posterior
	finaldens <- densY+densL+densM+densI+densWT+densW1
}
######################################################################################################
######################################################################################################
######################################################################################################
