...
#Labor Decision Rule
LX <- function(K, L, omega){
	Ldat <- cbind(K, L, omega)
	LX <- cbind(1, tensor(M=c(3,3,3), vars=Ldat)[,-1])
	return(LX)
}
#Derivative of Labor Rule
LXD <- function(K, L, omega, par, pos, sdpos){
	M <- c(3,3,3)
	Ldat <- cbind(K, L, omega)
...
WX1 <- function(K, L, M){
	W1dat <- cbind(K, L)
	W1dat <- cbind(1, tensor(M=c(3,3), vars=W1dat)[,-1])
	return(W1dat)
}
