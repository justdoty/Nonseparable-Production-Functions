...
# method <- "hermite"
#Load US dataset
US <- read.csv('NLPFQR/DATA/USdata.csv') %>% transmute(id=id, year=year, lny=log(Y), lnva=log(VA), lnk=log(K), lnl=log(L), lnm=log(M), lni=log(I), A=age)
...
#Save Results
ext <- paste("NLPFQR/DATA/Nonparametric/Bootstrap/NL_Boot_NPAR_", method, id, ".Rdata", sep="")
save(Rlist, file=ext)
