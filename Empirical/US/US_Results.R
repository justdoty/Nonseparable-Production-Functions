...
#Saves to file
print(summary_table, hline.after=c(0,nrow(summary_table)), add.to.row=addtorow, auto=FALSE, include.rownames=FALSE, sanitize.text.function=function(x) x, table.placement="H", file="/Users/justindoty/Documents/Research/Dissertation/Nonlinear_Production_Function_QR/Code/Empirical/US/Results/US_Summary.tex")
########################################################################################################
##########################################Load Results############################################
########################################################################################################
NAICS <- c("31", "32", "33")
for (i in 1:length(NAICS)){
	...
	for (t in 2:T){
		...
		lnidata[,t] <- cbind(1, lnkdata[,t], omgdata[,t], lnkdata[,t]*omgdata[,t], lnkdata[,t]^2, omgdata[,t]^2)%*%as.matrix(parI[-length(parI)])
		lnkdata[,t] <- log(dp$dp[t-1]*exp(lnkdata[,t-1])+exp(lnidata[,t-1])+exp(upsdata[,t-1]))
	}
	#Labor and Materials
	for (s in 1:T){
		lnldata[,t] <- rowSums(cbind(1, lnkdata[,t], omgdata[,t], lnkdata[,t]*omgdata[,t], lnkdata[,t]^2, omgdata[,t]^2)*lspline(vectau=vectau, bvec=parL, b1=parLb[1], bL=parLb[2], u=epsdata[,t]))
		lnmdata[,t] <- rowSums(cbind(1, lnkdata[,t], omgdata[,t], lnkdata[,t]*omgdata[,t], lnkdata[,t]^2, omgdata[,t]^2)*lspline(vectau=vectau, bvec=parM, b1=parMb[1], bL=parMb[2], u=varepsdata[,t]))
		lnydata[,t] <- rowSums(translog(K=lnkdata[,t], L=lnldata[,t], M=lnmdata[,t], omega=omgdata[,t])*lspline(vectau=vectau, bvec=parY, b1=parYb[1], bL=parYb[2], u=etadata[,t]))+omgdata[,t]
	}
	#Vectorize
	out <- c(t(lnydata))
	cap <- c(t(lnkdata))
	...
	save_plot(paste("/Users/justindoty/Documents/Research/Dissertation/Nonlinear_Production_Function_QR/Code/Empirical/US/Results/K_AQME_NAICS_", NAICS[i], ".png", sep=""), kaqme_plot)
	#Labor
	...
	save_plot(paste("/Users/justindoty/Documents/Research/Dissertation/Nonlinear_Production_Function_QR/Code/Empirical/US/Results/L_AQME_NAICS_", NAICS[i], ".png", sep=""), laqme_plot)
	#Materials
	...
	save_plot(paste("/Users/justindoty/Documents/Research/Dissertation/Nonlinear_Production_Function_QR/Code/Empirical/US/Results/M_AQME_NAICS_", NAICS[i], ".png", sep=""), maqme_plot)
	#Hicks-Capital
	...
	save_plot(paste("/Users/justindoty/Documents/Research/Dissertation/Nonlinear_Production_Function_QR/Code/Empirical/US/Results/HICKS_K_AQME_NAICS_", NAICS[i], ".png", sep=""), hkaqme_plot)
	#Hicks-Labor
	...
	save_plot(paste("/Users/justindoty/Documents/Research/Dissertation/Nonlinear_Production_Function_QR/Code/Empirical/US/Results/HICKS_L_AQME_NAICS_", NAICS[i], ".png", sep=""), hlaqme_plot)
	#Hicks-Labor
	...
	save_plot(paste("/Users/justindoty/Documents/Research/Dissertation/Nonlinear_Production_Function_QR/Code/Empirical/US/Results/HICKS_M_AQME_NAICS_", NAICS[i], ".png", sep=""), hmaqme_plot)
	...
	save_plot(paste("/Users/justindoty/Documents/Research/Dissertation/Nonlinear_Production_Function_QR/Code/Empirical/US/Results/OMG_AQME_NAICS_", NAICS[i], ".png", sep=""), omgaqme_plot)
	###############
	#Density Plots
	##############
	omgdens_plot <- ggplot(data.frame(omg), aes(x=omg)) + geom_density() + xlab("Productivity") + ylab("")
	save_plot(paste("/Users/justindoty/Documents/Research/Dissertation/Nonlinear_Production_Function_QR/Code/Empirical/US/Results/OMG_DENS_NAICS_", NAICS[i], ".png", sep=""), omgdens_plot)
}